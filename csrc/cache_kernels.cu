...
__global__ void copy_blocks_kernel(int64_t* key_cache_ptrs,
  ...
  // INTERVIEW PROBLEM 3: KV-Cache Block Copy
  //
  // Copy cache blocks from source to destination locations.
  // - key_cache_ptrs[layer] and value_cache_ptrs[layer] are pointers to each
  //   layer's cache (stored as int64_t, cast to scalar_t*)
  // - block_mapping contains [src, dst] pairs: block_mapping[2*i], block_mapping[2*i+1]
  // - Cache layout: cache[block_num * numel_per_block + element]
  //
  // Copy both key and value caches for the layer/pair this block handles.
