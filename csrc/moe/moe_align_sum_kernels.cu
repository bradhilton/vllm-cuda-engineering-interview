...
__global__ void moe_sum_kernel(
    ...
    const int d) {
  // INTERVIEW PROBLEM 2: MoE Sum Kernel
  //
  // Sum the outputs from TOPK experts for each token.
  // Grid: one block per token. Threads stride through dimension d.
  //
  // Use VLLM_LDG() for cached loads.
